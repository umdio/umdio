  ...
  module UMDIO
    module Helpers
      ...
      def check_semester(_app, semester)
        ...
        if Course.where(semester: semester).to_a.length.zero?
      ...
      def find_courses_v1(semester, course_ids, params)
        ...
        courses = Course.where(semester: semester, course_id: course_ids).map(&:to_v1)
        # check if found
        if courses.empty?
      ...
      def find_courses(semester, course_ids, params)
        ...
        courses = Course.where(semester: semester, course_id: course_ids).map(&:to_v0)
        # check if found
        if courses.empty?
          s = course_ids.length > 1 ? 's' : ''
          halt 404, {
