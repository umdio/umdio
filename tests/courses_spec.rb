...
describe 'Courses Endpoint' do  # Probably should be moved higher up the test ladder. For now!
  url = '/v0/courses'
  enes100_response_string = '{"course_id":"ENES100","name":"Introduction to Engineering Design","dep_abbr":"ENES","department":"Engineering Science","semester":"201501","credits":"3","grading_method":["Regular","Pass-Fail","Audit"],"core":["PS"],"gen_ed":["DSSP"],"description":"Corequisite: MATH140.Students work as teams to design and build a product using computer software for word-processing, spreadsheet, CAD, and communication skills.","course_relationships":{"coreqs":["Corequisite: MATH140"],"prereqs":[],"restrictions":[],"restricted_to":[],"credit_only_granted_for":[],"credit_granted_for":[],"formerly":[],"also_offered_as":[]},"sections":["ENES100-0101","ENES100-0201","ENES100-0202","ENES100-0301","ENES100-0302","ENES100-0401","ENES100-0501","ENES100-0502","ENES100-0601","ENES100-0602","ENES100-0801"]}'  
  ...
  describe 'Listing courses' do
    shared_examples_for 'get course list' do |url|
      ...
      it 'returns a list of courses' do
        ...
        expect(res[0]["course_id"]).to be == "AASP100"
        expect(res[1]["name"]).to be == "Introduction to African American Studies"
        expect(res[4260]["department"]).to be == "Women's Studies"
    ...
    describe 'GET /courses' do #this test takes most of the time in our test suite right now (80%)
      ...
      it 'returns full objects' do
        require 'json'
        expect(JSON.parse((get url).body)[1801].to_json).to be == enes100_response_string
      end
    end
